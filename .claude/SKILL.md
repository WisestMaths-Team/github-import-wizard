...
Core workflow
Locate the file. The user will either pass a path or reference a topic code (y2df3 → look for y2df3.ts in the current repo). If ambiguous, ask once.
Read the file. Parse out the Question[] array. You don't need to evaluate the TypeScript — read it as text and extract each question object.
For every question, independently work the problem from the stem. Do not trust the existing solution while doing this — derive your own answer first, then compare.
For every step in solutionSteps, check that the LaTeX in that step is a valid consequence of the LaTeX in the previous step (or the question stem, for step 1).
Classify any discrepancy by severity (see below).
Print the report to stdout in the exact format specified in references/report-format.md. If there are zero problems, print a single-line confirmation and stop.
...
Read only the question stem for the current question.
Solve it yourself, writing out your working in a scratchpad (a comment block, or just internally — but actually do the working, don't eyeball it).
Then compare your finalAnswer to the file's finalAnswer. If they differ, the file is wrong (or your working is — recheck once). If they match, walk the file's workingLatex steps to verify each transition.
For Challenge questions where there are multiple valid forms of the answer (e.g. \\tfrac{1}{2}\\ln(2x)+C vs \\tfrac{1}{2}\\ln x + \\tfrac{1}{2}\\ln 2 + C), check that the file's form is equivalent to yours, not literally identical.
